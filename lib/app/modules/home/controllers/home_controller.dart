...
class HomeController extends GetxController {
  ...
  Future<bool> _canCheckIn(Task currentTask) async {
    ...
    if (parentId == "N/A" || parentId.isEmpty) {
      print('DEBUG: Task ${currentTask.id} has no Parent ID or ID is invalid default. Check-in allowed.');
      return true;
    }
    ...
    if (parentTask == null) {
      print('DEBUG: Parent Task ID $parentId not found in list. Check-in allowed.');
    ...
    if (isParentCompleted) {
      ...
      print('DEBUG: Parent Task $parentId is NOT COMPLETED (Status: ${parentTask.taskStatus}). Check-in BLOCKED.');
      Get.snackbar(
        "Action Blocked",
